...
void copy_mat (double *u, double *v, unsigned sizex, unsigned sizey) {
    ...
    for (int blocki=0; blocki<nblocksi; ++blocki) {
      ...
      for (int blockj=0; blockj<nblocksj; ++blockj) {
        ...
        tareador_start_task("One task per block COPY_MAT");
        for (int i=max(1, i_start); i<=min(sizex-2, i_end); i++)
          for (int j=max(1, j_start); j<=min(sizey-2, j_end); j++)
            v[i*sizey+j] = u[i*sizey+j];
        tareador_end_task("One task per block COPY_MAT");
      }
    }
}
...
double solve (double *u, double *unew, unsigned sizex, unsigned sizey) {
    ...
    for (int blocki=0; blocki<nblocksi; ++blocki) {
      ...
      for (int blockj=0; blockj<nblocksj; ++blockj) {
        ...
        tareador_start_task("One task per block SOLVE");
        for (int i=max(1, i_start); i<=min(sizex-2, i_end); i++) {
          for (int j=max(1, j_start); j<=min(sizey-2, j_end); j++) {
        ...
        tareador_end_task("One task per block SOLVE");
