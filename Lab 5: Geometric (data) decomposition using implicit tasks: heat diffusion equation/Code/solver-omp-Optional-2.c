...
double solve (double *u, double *unew, unsigned sizex, unsigned sizey) {
    ...
    #pragma omp single
    {
        int control[nblocksi+1][nblocksj+1];
            
            for(int i = 0; i<= nblocksi; ++i){
                for(int j = 0; j <= nblocksj; ++j){
                    control[i][j]=0; //not finished tasks
                }
            }
        #pragma omp taskloop private(tmp, diff) shared(control) reduction(+: sum)
        for (int blocki = 0; blocki < nblocksi; blocki++){
            ...
            for (int blockj=0; blockj<nblocksj; ++blockj) {
                ...
                // First row of blocks can be computed directly
                if(blocki != 0){
                    do{
                        #pragma omp atomic read
                        temp1 = control[blocki-1][blockj];
                    }while(temp1 == 0);
                }
                ...
                control[blocki][blockj]=1; //finished tasks
