...
void mandelbrot(int height, int width, double real_min, double imag_min,
    ...
    for (int row = 0; row < height; ++row) {
        for (int col = 0; col < width; ++col) {
            tareador_start_task("point-mandel");
            complex z, c;
	    ...
	    output[row][col]=k;
            ...
            tareador_end_task("point-mandel");
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i=1; i<argc; i++) {
	      if (strcmp(argv[i], "-d")==0) {
			      ...
			      size = atof(argv[++i]);
                              ...
                              output2file = 1;
    			      if((fp=fopen("output_tar.out", "wb"))==NULL) { 
    ...
    if (output2histogram) histogram = malloc(maxiter*sizeof(int));
    ...
    if ((output2file) && (fp != NULL)) {
        fprintf(stdout, "Writing output file to disk: output_tar.out\n");
        for (int row = 0; row < height; ++row)
