...
void merge(long n, T left[n], T right[n], T result[n*2], long start, long length, long depth) {
...
void multisort(long n, T data[n], T tmp[n], long depth) {
    if (n >= MIN_SORT_SIZE*4L) {
        ...
        if(!omp_in_final())
        {
            ...
            #pragma omp task final (depth >= CUTOFF) depend(in: data[0], data[3L*n/4L]) depend(out: tmp[n/2L])
