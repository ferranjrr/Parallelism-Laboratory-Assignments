...
void merge(long n, T left[n], T right[n], T result[n*2], long start, long length) {
    if (length < MIN_MERGE_SIZE*2L) {
        ...
        tareador_start_task("Merge_tree_1_rec");
        merge(n, left, right, result, start, length/2);
        tareador_end_task("Merge_tree_1_rec");
        
        tareador_start_task("Merge_tree_2_rec");        
        merge(n, left, right, result, start + length/2, length/2);
        tareador_end_task("Merge_tree_2_rec");        
    }
}
...
void multisort(long n, T data[n], T tmp[n]) {
    if (n >= MIN_SORT_SIZE*4L) {
        // Recursive decomposition
        tareador_start_task("multisort_tree_1");        
        multisort(n/4L, &data[0], &tmp[0]);
        tareador_end_task("multisort_tree_1");
        tareador_start_task("multisort_tree_2");
        multisort(n/4L, &data[n/4L], &tmp[n/4L]);
        tareador_end_task("multisort_tree_2");
        tareador_start_task("multisort_tree_3");
        multisort(n/4L, &data[n/2L], &tmp[n/2L]);
        tareador_end_task("multisort_tree_3");
        tareador_start_task("multisort_tree_4");
        multisort(n/4L, &data[3L*n/4L], &tmp[3L*n/4L]);
        tareador_end_task("multisort_tree_4");

        tareador_start_task("merge_tree_1");
        merge(n/4L, &data[0], &data[n/4L], &tmp[0], 0, n/2L);
        tareador_end_task("merge_tree_1");
        tareador_start_task("merge_tree_2");
        merge(n/4L, &data[n/2L], &data[3L*n/4L], &tmp[n/2L], 0, n/2L);
        tareador_end_task("merge_tree_2");
